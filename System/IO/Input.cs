...
public static class Input
{
	...
	public static Keys[] Pressed => _pressed;

	private static volatile Keys[] _pressed = [];
	...
	internal static void Initialize(GameWindow window) => window.TextInput += OnTextInput;
	...
	internal static void Update(GameTime gameTime)
	{
		...
		_pressed = _currentKeyState.GetPressedKeys();
	}

	internal static int GetCharPressed() => _charQueue.Count > 0 ? _charQueue.Dequeue() : 0;

	internal static bool IsKeyPressed(Keys key) =>
		_currentKeyState.IsKeyDown(key) && _previousKeyState.IsKeyUp(key);

	internal static bool IsKeyDown(Keys key) => _currentKeyState.IsKeyDown(key);

	internal static bool IsKeyPressedRepeat(Keys key)
	{
